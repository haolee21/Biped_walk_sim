function [ceq,gradceq] = dynConst_discrete_nogrf(x,p)
% x = gpuArray(x);
...
gradceq = zeros(p.numJ*2,size(x,2),size(ceq,1));
for i=1:size(x,2)-2
...
%     
    ...
    ceq((i-1)*p.numJ+1:i*p.numJ,1) = dL1+dL2+u_sum+tend_ank1+tend_ank2+tend_kne1+tend_kne2-fri_tau;
