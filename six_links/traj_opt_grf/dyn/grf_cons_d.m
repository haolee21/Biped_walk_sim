...
for i=p.phase1_idx+2:size(x,2)-1 %we only have totT-1 constraints since all velocity is generated by (q2-q1)/sampT
    ...
    q = (q1+q2)/2;
    dq = (q2-q1)/p.sampT;
    fy_toe1 = x(2*p.numJ+3,i);
    fy_toe2 = x(2*p.numJ+3,i+1);
    
    
    fx_toe = (fx_toe1+fx_toe2)/2;
    fx_heel =(fx_heel1+fx_heel2)/2;
    fy_toe = (fy_toe1+fy_toe2)/2;
    ...
    if(i>p.phase1_idx+2)
        ...
        grf_toe_c2(i,1)=fx_toe^2-p.us^2*fy_toe^2;
    end
    ...
    if nargout>2
        ...
        if(i>p.phase1_idx+2)
            ...
            dgrf_toe_c2(2*p.numJ+1,i,i) = dgrf_toe_c2(2*p.numJ+1,i,i)+fx_toe;
            dgrf_toe_c2(2*p.numJ+1,i+1,i) = dgrf_toe_c2(2*p.numJ+1,i+1,i)+fx_toe;
            dgrf_toe_c2(2*p.numJ+3,i,i)=dgrf_toe_c2(2*p.numJ+3,i,i)-p.us^2*fy_toe;
            dgrf_toe_c2(2*p.numJ+3,i+1,i)=dgrf_toe_c2(2*p.numJ+3,i+1,i)-p.us^2*fy_toe;
            
        end
        
        
        dgrf_toe_c4(1:p.numJ,i,i)=dGrf_toe_c4_q1(q.',dq.',fx_toe,p.sampT);
        dgrf_toe_c4(1:p.numJ,i+1,i)=dGrf_toe_c4_q2(q.',dq.',fx_toe,p.sampT);
        dgrf_toe_c4(2*p.numJ+1:2*p.numJ+2,i,i)=dGrf_toe_c4_F(q.',dq.')*0.5;
        dgrf_toe_c4(2*p.numJ+1:2*p.numJ+2,i+1,i)=dGrf_toe_c4_F(q.',dq.')*0.5;
        
        dgrf_heel_c4(1:p.numJ,i,i)=dGrf_heel_c4_q1(q.',dq.',fx_heel,p.sampT);
        dgrf_heel_c4(1:p.numJ,i+1,i)=dGrf_heel_c4_q2(q.',dq.',fx_heel,p.sampT);
        dgrf_heel_c4(2*p.numJ+2,i,i)=dGrf_heel_c4_F(q.',dq.')*0.5;
        dgrf_heel_c4(2*p.numJ+2,i+1,i)=dGrf_heel_c4_F(q.',dq.')*0.5;
