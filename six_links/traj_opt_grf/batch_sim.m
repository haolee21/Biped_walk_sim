...
models = {'human_9','human_9_load','human_9_exo_load'};
...
task_i = 1; 
tasks = cell(1,size(models,2)*size(weights,2)*size(hipLen,2)*size(knee_dirs,2));

for i1=1:size(models,2)
    for i2=1:size(weights,2)
        for i3=1:size(hipLen,2)
            for i4=1:size(knee_dirs,2)
                tasks{1,task_i}=@()batch_sim_single(models{1,i1},hipLen(1,i3),weights(i2,:),knee_dirs(1,i4));
                task_i = task_i+1;
            end
...
const_val = zeros(1,size(outputs,2));
...
T = table(fileNames.',modelNames.',hipLens.',w.',knee_dir.',const_val.');
