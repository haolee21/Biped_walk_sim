function Tau_toe_pushoff = Tau_toe_pushoff(in1,in2,Fx,Fy,H,k,cmax,dmax,sampT)
%TAU_TOE_PUSHOFF
%    TAU_TOE_PUSHOFF = TAU_TOE_PUSHOFF(IN1,IN2,FX,FY,H,K,CMAX,DMAX,SAMPT)

%    This function was generated by the Symbolic Math Toolbox version 8.5.
%    04-Dec-2020 16:39:40

q_t1 = in1(:,1);
q_t2 = in1(:,2);
q_t3 = in1(:,3);
q_t4 = in1(:,4);
q_t5 = in1(:,5);
q_t6 = in1(:,6);
t2 = q_t1+q_t2;
t13 = atan(5.18251222829003e-1);
t14 = atan(1.929566117647059);
t15 = 1.903089174117337e+18;
t3 = cos(t2);
t4 = sin(t2);
t5 = q_t3+q_t4+t2;
t20 = -t14;
t6 = cos(t5);
t7 = q_t5+t5;
t8 = sin(t5);
t11 = t3.*4.453122e-1;
t12 = t4.*4.453122e-1;
t9 = sin(t7);
t10 = cos(t7);
t16 = t6.*4.453122e-1;
t17 = t8.*4.453122e-1;
t21 = q_t6+t7+t13;
t23 = q_t6+t7+t20;
t18 = t10.*4.60062e-1;
t19 = t9.*4.60062e-1;
t22 = cos(t21);
t24 = cos(t23);
t25 = (t15.*t22)./1.125899906842624e+19;
t26 = (t15.*t24)./1.125899906842624e+19;
t27 = t16+t18+t25;
t29 = t17+t19+t26;
t28 = Fy.*t27;
t30 = Fx.*t29;
t31 = -t30;
t32 = t28+t31;
Tau_toe_pushoff = [Fy.*(t11+t27+cos(q_t1).*4.60062e-1)-Fx.*(t12+t29+sin(q_t1).*4.60062e-1);-Fx.*(t12+t29)+Fy.*(t11+t27);t32;t32;-Fx.*(t19+t26)+Fy.*(t18+t25);Fy.*t25-(Fx.*t15.*t24)./1.125899906842624e+19];
