...
param.knee_stiff =0;%76.325; % I use max moment (MVC/angle), since the stiffness of the paper is too high
...
ub1 = [181/180*pi*ones(1,length(time)-1);
      181*ones(1,length(time)-1)/180*pi;
      90/180*pi*ones(1,length(time)-1);
      -100/180*pi*ones(1,length(time)-1);
      zeros(1,length(time)-1);
...
lb1 = [ones(1,length(time)-1)/180*pi;
      -0.0001*pi*ones(1,length(time)-1);
      -75/180*pi*ones(1,length(time)-1);
      -260/180*pi*ones(1,length(time)-1);
       -181*pi*ones(1,length(time)-1)/180*pi;
...
[t1,~]=clock;
fileName = [num2str(t1(2),'%02d'),num2str(t1(3),'%02d'),num2str(t1(4),'%02d'),num2str(t1(5),'%02d'),'_rev'];
result.x1 = x1;
