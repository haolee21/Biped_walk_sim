...
% the length data is from David A. Winter's Biomechanics and Motor Control of Human Movement 
...
h_heel = 0.039*totH;  %from Rudolfs Drillis (1964) Table.1, G
...
m_thigh = 0.1*totM;
m_head = 0.081*totM;
m_torso = 0.678*totM+20; %additional load carrying

% add additional weight of the leg
% each leg is around 5 kg
% m_exo_calf = 1.7;
% m_exo_thigh=1.7;
% m_exo_foot = 0.8;
% m_calf = m_calf+m_exo_calf;
% m_thigh = m_thigh+m_exo_thigh;
% m_foot = m_foot+m_exo_foot;
...
lc_foot = 0.5*l_foot-l_heel; % de Leva, shift by l_heel since the joint is not at the end of foot 
% include head when calculating, suppose neck is fixed
lc_torso = (0.5*l_torso*m_torso+(0.1166*totH*(1-0.5976)+l_torso)*m_head)/(m_head+m_torso); 




I_calf = [0,0,0;     %ROG data from "Comparison of direct collocation optimal control", same as what I got from David A. Winter's 
          0,0,0;
